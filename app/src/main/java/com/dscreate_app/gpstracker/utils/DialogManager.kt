    ...
    fun showSaveDialog(context: Context,item: TrackItem?, listener: Listener) {
        ...
        item?.let {
            with(binding) {
                ...
                val speed = "${String.format("%.1f", it.speed)} ${context.getString(R.string.meter_in_sec)}"
                val distance = "${String.format("%.1f", it.distance / 1000)} ${context.getString(R.string.distance_in_kilometer)}"
                tvTime.text = time
                tvSpeed.text = speed
